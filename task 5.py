...
for ite in range(iteration):
    ...
    for i in range(m):
        ...
        visited = list(rute[i, :-1].astype(int))
    ...
    for i in range(m):
        ...
        for j in range(n - 1):
            ...
            s += d[a, b]
        s += d[int(rute_opt[i, -1]) - 1, 0]
        dist_cost[i] = s

    dist_min_loc = np.argmin(dist_cost)
    dist_min_cost = dist_cost[dist_min_loc].item()
    ...
    for i in range(m):
        for j in range(n - 1):
            ...
            pheromone[a, b] += 1 / dist_cost[i].item()

print("Task 5: Implementation of Ant Colony Optimization to Optimize Ride-Sharing Trip Duration using Python.\n")
print("Final Routes of all Ants:")
print(rute_opt)
print("\nBest Route (Ride-Sharing Path):", best_route)
print("Optimal Trip Duration (minutes):", int(dist_min_cost))
